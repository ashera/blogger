...
const TABLE_DESCRIPTIONS: Record<string, TableMeta> = {
  ...
  blog_seeds: {
    group: "Blog Builder",
    desc: "Top-level blog seeds — one subject per row. Holds wizard progress, SERP analysis, and the per-user link.",
  },
  blog_keywords: {
    group: "Blog Builder",
    desc: "Per-seed keyword set (starter + AI-expanded). One row flagged primary anchors the seed.",
  },
  blog_clusters: {
    group: "Blog Builder",
    desc: "The keyword cluster for a seed (one per seed) — name + intent.",
  },
  blog_seed_images: {
    group: "Blog Builder",
    desc: "Pexels image pool per seed (slots 0–4 primary + custom-keyword extras).",
  },
  blog_instances: {
    group: "Blog Builder",
    desc: "Generated blog instances for a seed — each a distinct post; chosen_attempt_id points at the kept generation.",
  },
  blog_generation_attempts: {
    group: "Blog Builder",
    desc: "Regeneration history — one row per generate/re-roll of an instance (success or error).",
  },
  blog_builder_settings: {
    group: "Blog Builder",
    desc: "Tunable prompt budgets and max_tokens caps used during blog generation. Single-row table managed at /admin/builder-settings.",
  },
    ...
    >
      ...
      <div style={{ textAlign: "right", whiteSpace: "nowrap", minWidth: 80 }}>
        {row.error ? (
          <span
            title={row.error}
            style={{ color: "var(--danger-700)", fontSize: 12, fontWeight: 600 }}
          >
            error
          </span>
        ) : (
