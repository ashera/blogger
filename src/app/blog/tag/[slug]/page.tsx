...
function authorLabel(p: PostRow): string {
  ...
  if (p.author_email) {
    return p.author_email.split("@")[0] ?? "blogger";
  }
  return "blogger";
}
...
export async function generateMetadata({
  ...
  if (!tag) return { title: "Tag not found · blogger" };
  const baseUrl = await getBaseUrl();
  const description = `Posts tagged ${tag.label} on the blogger blog.`;
  return {
    title: `${tag.label} · blogger blog`,
    description,
