...
const ALLOWED_HERO_MIMES = new Set(["image/jpeg", "image/png", "image/webp"]);
...
/** Redirect to the post list unless this post belongs to the user. */
async function assertPostOwned(postId: string, userId: string): Promise<void> {
  const r = await query<{ one: number }>(
    `SELECT 1 AS one FROM blog_posts
      WHERE id = $1::bigint AND author_id = $2::bigint LIMIT 1`,
    [postId, userId],
  );
  if (r.rows.length === 0) redirect(POSTS);
}
...
export async function createBlogPost(formData: FormData): Promise<void> {
  ...
  if (!title) redirect(`${POSTS}/new?error=invalid-title`);
  ...
  if (customSlug && !isValidSlug(customSlug)) {
    redirect(`${POSTS}/new?error=invalid-slug`);
  }
...
export async function updateBlogPost(formData: FormData): Promise<void> {
  ...
  if (!/^\d+$/.test(id)) redirect(POSTS);
  await assertPostOwned(id, me.id);

  const title = getString(formData, "title", TITLE_MAX);
  if (!title) redirect(`${POSTS}/${id}/edit?error=invalid-title`);

  const customSlug = getString(formData, "slug", SLUG_MAX);
  const baseSlug = customSlug ? customSlug : slugify(title);
  if (customSlug && !isValidSlug(customSlug)) {
    redirect(`${POSTS}/${id}/edit?error=invalid-slug`);
  }
...
// Tags are a shared global taxonomy any signed-in user can extend.
export async function createBlogTag(formData: FormData): Promise<void> {
  await requireUser(`${POSTS}/tags`);
  const label = String(formData.get("label") ?? "").trim();
  if (!label) redirect(`${POSTS}/tags?error=invalid-label`);
...
/**
 * Fire-and-forget view logger called from the public post page. Skips admin
 * viewers. Errors are swallowed — analytics shouldn't break rendering.
 */
