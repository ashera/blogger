...
export async function dispatchEmailChangeRequest(
  ...
  const send = await sendEmail({
    to: newEmail,
    subject: "Confirm your new blogger email",
    html: emailLayout({
      preheader: "Confirm the new email on your blogger account.",
      heading: "Confirm your new email",
      body: `
        <p>You asked to change your blogger login email from <strong>${escapeHtml(currentEmail)}</strong> to <strong>${escapeHtml(newEmail)}</strong>. Click the button below to make the switch — the link expires in 24 hours.</p>
